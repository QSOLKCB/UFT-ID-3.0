...
def _live_roadmap_errors() -> list[str]:
    errors=[]
    try: roadmap=_strict_live_roadmap(_frozen.PATHS["roadmap"])
    except (OSError, ValueError) as exc: return [f"information live roadmap JSON invalid: {exc}"]
    if roadmap.get("schema_version") != "1.7.0": errors.append("information live roadmap schema drift")
    if roadmap.get("basis_commit") != "516cff5d6a45af54d6fc4ae9c72c2e8e9c668637": errors.append("information live roadmap basis commit must be merged EFP PR #19")
    if roadmap.get("active_planned_surface") != 10:
        errors.extend(["information live roadmap active surface must be PR #10","information live roadmap active surface must be PR #17","information live roadmap active surface must be PR #16","information roadmap active surface must be PR #15"])
    if roadmap.get("completed") != [5,6,7,8,9,11,12,13,14,15,16,17,18]: errors.append("information live roadmap completed set drift")
    if roadmap.get("deferred") != []: errors.append("information live roadmap deferred set drift")
    ...
    if not isinstance(sequence,list): errors.append("information live roadmap sequence malformed")
        ...
        if by_pr.get(18,{}).get("status") != "complete-merged-516cff5d6a45af54d6fc4ae9c72c2e8e9c668637": errors.append("information live roadmap PR18 completion drift")
