#!/usr/bin/env python3
"""Live compatibility wrapper for the merged Recovery Specializations authority.

The exact Recovery validator merged in GitHub PR #17 is preserved in
validate_recovery_specializations_pr17_frozen.py. This wrapper replays that
authority against its historical PR16-active roadmap state, then independently
checks the current planned PR18 Empirical Falsification Profile live schedule.
Recovery theorem, counterexample, contract, and finite evidence semantics remain
frozen.
"""
...
FROZEN = ROOT / "scripts/validate_recovery_specializations_pr17_frozen.py"
ROADMAP_STATE = ROOT / "machine/roadmap_state.json"

_spec = importlib.util.spec_from_file_location("recovery_validator_pr17_frozen", FROZEN)
if _spec is None or _spec.loader is None: raise RuntimeError(f"cannot load frozen Recovery validator: {FROZEN}")
...
HISTORICAL_ROADMAP_STATE = {
    "type":"uft-id-roadmap-state","schema_version":"1.4.0","snapshot_date":"2026-08-24","basis_commit":"22b589c4e2e2042d180d64db837f092a007e0813",
    "completed":[5,6,7,8,9,11,12,13,14,15],"active_planned_surface":16,"deferred":[10],
    "sequence":[
...
def _live_roadmap_errors() -> list[str]:
    ...
    for token in _frozen.PRIVATE_PATTERNS:
        if token.casefold() in serialized: errors.append(f"recovery live roadmap contains forbidden private locator: {token}")
    return errors
...
def main() -> int:
    ...
    elif result["status"]=="ok": print(f"Recovery Specializations authority: ok ({result['result_count']} results, {result['boundary_count']} hard boundaries)")
    else:
