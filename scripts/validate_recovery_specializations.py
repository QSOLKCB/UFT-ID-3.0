...
checks the current planned PR17 Continuum/Stochastic/Prevalence live schedule.
...
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"cannot load frozen Recovery validator: {FROZEN}")
...
def _live_roadmap_errors() -> list[str]:
    ...
    if roadmap.get("schema_version") != "1.5.0":
        errors.append("recovery live roadmap schema drift")
    if roadmap.get("basis_commit") != "2f2cdd2af195a2e74a55e14abfbc4f88e0901a8f":
        errors.append("recovery live roadmap basis commit must be merged Recovery PR")
    if roadmap.get("active_planned_surface") != 17:
        errors.append("recovery live roadmap active surface must be PR #17")
        errors.append("recovery roadmap active surface must be PR #16")
    if roadmap.get("completed") != [5, 6, 7, 8, 9, 11, 12, 13, 14, 15, 16]:
        errors.append("recovery live roadmap completed set drift")
    if roadmap.get("deferred") != [10]:
        errors.append("recovery live roadmap deferred set drift")
    sequence = roadmap.get("sequence")
    if not isinstance(sequence, list):
        errors.append("recovery live roadmap sequence malformed")
    else:
        ...
        if by_pr.get(16, {}).get("status") != "complete-merged-2f2cdd2af195a2e74a55e14abfbc4f88e0901a8f":
            errors.append("recovery live roadmap PR16 completion drift")
        if by_pr.get(17, {}).get("status") != "active-implemented-in-current-change":
            errors.append("recovery live roadmap PR17 active-state drift")
        if by_pr.get(18, {}).get("status") != "planned":
            errors.append("recovery live roadmap PR18 must remain planned")
    ...
    if not isinstance(rules, list) or csp_rule not in rules:
        errors.append("recovery live roadmap CSP obligation rule missing")
    ...
    for token in _frozen.PRIVATE_PATTERNS:
        if token.casefold() in serialized:
            errors.append(f"recovery live roadmap contains forbidden private locator: {token}")
    return errors
