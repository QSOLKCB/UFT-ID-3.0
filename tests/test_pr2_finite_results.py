...
class FiniteEntropySignTests(unittest.TestCase):
    ...
    def test_deterministic_merge_decreases_by_one_bit(self):
        ...
        self.assertTrue(math.isclose(case["delta_H_bits"], -1.0, abs_tol=1e-12))


class RepresentationTests(unittest.TestCase):
    def test_same_fine_trajectory_has_opposite_observed_signs(self):
        result = COARSE.run()
        self.assertTrue(math.isclose(result["fine_delta_H_bits"], 0.0, abs_tol=1e-12))
...
class PolygonExtremumTests(unittest.TestCase):
    def test_n6_n2_extremum_is_reversed_from_a_minimum_claim(self):
        result = POLYGON.run(max_N=6, max_n=2)
        case = result["triangle_scale_N6_n2"]
        self.assertEqual(case["maximum"]["counts"], [3, 3])
        self.assertEqual(case["minimum"]["counts"], [1, 5])
        self.assertTrue(math.isclose(case["maximum"]["H_bits"], 1.0, abs_tol=1e-12))
    ...
    def test_balanced_counts_are_exhaustive_maxima_in_small_range(self):
        ...
        self.assertGreater(len(result["exhaustive_cases"]), 0)
        for case in result["exhaustive_cases"]:
