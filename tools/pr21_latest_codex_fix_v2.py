...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if text.count(old) != 1:
        raise RuntimeError(f"{path}: replacement anchor drift: {old[:120]!r}")
    p.write_text(text.replace(old, new, 1), encoding="utf-8")


def wrapper_source() -> str:
    source_dict = "\n".join(f'    {path!r}: {sha!r},' for path, sha in SOURCE_BLOBS)
...
ROOT = Path(__file__).resolve().parents[1]
FROZEN = ROOT / {FROZEN_VALIDATOR!r}
...
def validate_documents(freeze, source_theorems, source_counterexamples, base_contract, human, roadmap, readme, *, check_paths: bool = True):
    ...
    expected_authority = {{
        ...
        "frozen_validator": {FROZEN_VALIDATOR!r},
...
def main() -> int:
    live_validator = ROOT / "scripts/validate_lean_observation_foundation.py"
    frozen_validator = ROOT / FROZEN_VALIDATOR
    if frozen_validator.exists():
        raise RuntimeError(f"frozen validator already exists: {FROZEN_VALIDATOR}")
    shutil.copyfile(live_validator, frozen_validator)
    ...
    if not isinstance(authority, dict):
        raise RuntimeError("lean observation authority missing")
    authority["workflow"] = ".github/workflows/vopson-corpus.yml"
    authority["frozen_validator"] = FROZEN_VALIDATOR
    # Preserve a deterministic key order for the authority object.
    authority_order = [
        "machine_contract", "human", "validator", "frozen_validator", "tests",
        "source_theorems", "source_counterexamples", "source_observation_contract",
        "workflow", "rule",
    ]
    contract["lean_observation_foundation_authority"] = {k: authority[k] for k in authority_order}
    ...
    live_validator.write_text(wrapper_source(), encoding="utf-8")
    ...
    if tests.count(marker) != 1:
        raise RuntimeError("test insertion marker drift")
...
'''
    ...
    print(json.dumps({"status": "patched-v2", "source_count": len(SOURCE_BLOBS), "frozen_validator": FROZEN_VALIDATOR}, indent=2))
    return 0
